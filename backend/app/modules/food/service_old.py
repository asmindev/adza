    ...
    def create_food(
        ...
        if images and len(images) > 0:
            try:
                ...
                # Note: In production, you might want to rollback the food creation
            ...
            try:
                ...
                if image and image.food_id == food_id:  # Security check
                    ...
                    image_path = os.path.join(
                        FoodService._get_food_image_directory(food_id),
                        image.filename
                    )
            ...
            if images:
                for image in images:
                    try:
                        ...
                        logger.error(f"Error cleaning up image {image.filename}: {str(e)}")
        except Exception as e:
...
class FoodService:
    ...
    def get_all_foods_with_details(
        ...
        validation_result = FoodValidator.validate_pagination_params(page, limit)
        ...
        # Use data service to get enriched food data
        foods_dict = []
        for food in result["items"]:
    ...
    def get_food_detail(food_id: str) -> Optional[Dict[str, Any]]:
        """Get single food with full details"""
        # Validate food ID
        validation_result = FoodValidators.validate_food_id(food_id)
        if not validation_result["is_valid"]:
            raise ValueError(validation_result["error"])
    ...
    def create_food(
        ...
        validation_result = FoodValidators.validate_food_creation_data(food_data)
        if not validation_result["is_valid"]:
            raise ValueError(validation_result["error"])

        # Validate images if provided
        if images:
            images_validation = FoodValidators.validate_food_images(images)
            if not images_validation["is_valid"]:
                raise ValueError(images_validation["error"])
        ...
        if images and len(images) > 0:
            try:
                ...
                logger.error(f"Error uploading images for food {food.id}: {str(e)}")
                # Note: In production, you might want to rollback the food creation
                # or at least log this as a critical error

        # Return full details - use existing food object with base data
        result_food = FoodDataService.get_food_with_details(food.id)
        return result_food if result_food else food.to_dict()
    ...
    def update_food(
        ...
        update_validation = FoodValidators.validate_food_update_data(data)
        if not update_validation["is_valid"]:
            raise ValueError(update_validation["error"])

        # Validate new images if provided
        if new_images:
            images_validation = FoodValidators.validate_food_images(new_images)
            if not images_validation["is_valid"]:
                raise ValueError(images_validation["error"])
    ...
    def search_foods(
        ...
        """Search foods with filters"""
        # Validate pagination
        validation_result = FoodValidators.validate_pagination_params(page, limit)
        if not validation_result["is_valid"]:
            raise ValueError(validation_result["error"])
        ...
        # Use basic data for now since get_food_summary doesn't exist
        foods_dict = []
        for food in result["items"]:
            base_data = food.to_dict()
            foods_dict.append(base_data)
